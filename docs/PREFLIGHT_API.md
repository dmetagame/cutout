...
Milestone 1 rules, runs the unchanged `CUTOUT-v1.3` engine, and applies
`GUARD_POLICY-v1`.
...
Only `action: "shield"` is accepted. Extra fields, JSON numeric amounts,
unsupported tokens, invalid bounds, malformed addresses, and mismatched chain
or evaluation blocks fail closed.
...
also decimal base-unit strings.
...
When the indexer is in `ERROR`, the API returns its persisted operational code
and does not serve a previously complete snapshot.
