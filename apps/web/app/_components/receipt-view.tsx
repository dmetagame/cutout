    ...
    <>
      ...
      <main ref={motionScope} className="page-shell receipt-page motion-root">
        ...
        <section className="surface receipt-surface" aria-labelledby="receipt-title" data-receipt-reveal>
          ...
          <details className="evidence-disclosure receipt-disclosure" open><summary><span><span className="summary-kicker">Provenance</span> Inclusion and decision binding</span><span className="summary-meta">Public fields <ExternalLink size={14} /></span></summary><dl className="provenance-grid"><dt>Network</dt><dd>{artifact.chainId}</dd><dt>Pool</dt><dd>{artifact.pool}</dd><dt>Block hash</dt><dd>{artifact.blockHash}</dd><dt>Observed snapshot</dt><dd>{artifact.observedSnapshotHash}</dd><dt>Guard policy</dt><dd>{artifact.guardPolicyVersion}</dd><dt>Receipt timestamp</dt><dd>{new Date(artifact.timestamp * 1_000).toLocaleString()}</dd><dt>Receipt ID</dt><dd>{artifact.receiptId}</dd></dl></details>
